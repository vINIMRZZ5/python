#As Organizações Tabajara resolveram dar um aumento de salário aos seus colaboradores e lhe contrataram para desenvolver o programa que calculará os reajustes.
#Faça um programa que recebe o salário de um colaborador e o reajuste segundo o
#seguinte critério, baseado no salário atual:
#salários até R$ 280,00 (incluindo): aumento de 20%
#salários entre R$ 280,00 e R$ 700,00: aumento de 15%
#salários entre R$ 700,00 e R$ 1500,00: aumento de 10%
#salários de R$ 1500,00 em diante: aumento de 5% Após o aumento ser realizado,
#informe na tela:
#o salário antes do reajuste;
#o percentual de aumento aplicado;
#o valor do aumento; o novo salário, após o aumento.

sal = float(input("Dígite seu salário: "))
if sal <= 280:
    total = sal+((sal*20)/100)
    print("O aumento foi de 20%")
    print("Salário antes do reajuste: ", sal)
    print("Salário após o reajuste: ", total)
    print("Valor do aumento: ",sal*20/100)
elif (sal >280) and (sal<=700):
    total = sal+((sal*15)/100)
    print("O aumento foi de 15%")
    print("Salário antes do reajuste: ", sal)
    print("Salário após o reajuste: ", total)
    print("Valor do aumento: ",sal*15/100)
elif (sal >700) and (sal<=1500):
    total = sal+((sal*10)/100)
    print("O aumento foi de 10%")
    print("Salário antes do reajuste: ", sal)
    print("Salário após o reajuste: ", total)
    print("Valor do aumento: ",sal*10/100)
elif (sal >1500):
    total = sal+((sal*5)/100)
    print("O aumento foi de 5%")
    print("Salário antes do reajuste: ", sal)
    print("Salário após o reajuste: ", total)
    print("Valor do aumento: ",sal*5/100)
